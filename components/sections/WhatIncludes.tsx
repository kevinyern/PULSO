  ...
  return (
    <section className="reveal" style={{ padding: 'clamp(4rem, 8vw, 7rem) clamp(1.25rem, 5vw, 2rem)', borderTop: '1px solid #2a2a2a', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', position: 'relative', zIndex: 1 }}>
        ...
        <div className="grid-4col" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '0', border: '1px solid #2a2a2a' }}>
            ...
            <div key={block.title} style={{
              ...
              <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
                {block.items.map(item => (
                  <li key={item} style={{ color: '#ccc', fontSize: 'clamp(0.95rem, 2vw, 1.05rem)', lineHeight: 1.65, paddingLeft: '1rem', borderLeft: '1px solid #333' }}>{item}</li>
                ))}
              </ul>
            </div>
          ))}
